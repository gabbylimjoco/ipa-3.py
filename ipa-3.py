...
def relationship_status(from_member, to_member, social_graph):
    ...
    followee = social_graph[to_member]["following"]
    if to_member in follow:
        if from_member in followee:
...
def tic_tac_toe(board):
    ...
    for col in range(len(board[0])):
        if all(board[row][col] == board[0][col] for row in range(len(board))):
            return(board[0][col])
    if all(board[tac][tac] == board[0][0] for tac in range(len(board))):
        return board[0][0]
    tic = len(board)
    if all(board[tac][tic - tac -1] == board[o][-1] for tac in range(len(board))):
        return board[0][-1]
...
def eta(first_stop, second_stop, route_map):
    ...
    while temporary_stop != second_stop:
        ...
        if route in route_map:
            ...
            next_stop = None
            for r in route_map:
                if r[0] == temporary_stop:
                    next_stop = r[1]
                    break
            if next_stop is not None:
                total_time += route_map[(temporary_stop, next_stop)]['travel_time_mins']
